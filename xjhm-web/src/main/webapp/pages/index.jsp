...
<script>
        $.ajaxSetup({cache:false});//设置ajax默认参数，不缓存.
        var contextPath = '${pageContext.request.contextPath}';
        //var baseUrl = contextPath + "/KuBizCourseManage/";//20150720am
            ...
            $('#userManager').find("li[data-target=#rolesToggle]").click(function(){
                $.get(contextPath + '/pages/auth/role-switch.jsp').done(function(data){
                    ...
                    dialog.find('#toggle').on('click',function(){
                        ...
                        if(items.length>1){
                            dialog.find('.selectRoleGrid').message({type:'warning', content:'只能选择一个角色'});
                            return;
                        }
                        $.post(contextPath + '/auth/currentUser/switchOverRoleOfUser.koala', "roleName=" + items[0].name,function(data){
                            if(data.success){
                                dialog.find('.selectRoleGrid').message({type:'success', content:'切换角色成功！'});
                                window.location.href=contextPath+"/index.koala";
                            }else{
                                dialog.find('.selectRoleGrid').message({type:'error', content:'系统错误！'});
                            }
                        });
